...
let token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjEiLCJpYXQiOjE3MjcxMjE5NDl9.OzC6hrSL7NBjIqzYr2SoDpsK0xTGMOUzraMX5dDDfEQ';
const product = {
    id: `${UUID}`,
    name: "for",
    price: 1,
...
describe(' users endpoints', () => {
    ...
    it(' / authenticate the user and gets a token ', async () => {
        ...
        expect(res.status).toBe(200);
        const { id, firstname: firstName, lastname: lastName } = res.body.authen;
...
describe('order endpoint', () => {
    it('s / hould get spicefic order', async () => {
        const res = await request
